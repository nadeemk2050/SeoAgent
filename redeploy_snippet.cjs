...
async function run() {
  try {
    ...
    // Deactivate old conflicting snippets by ID
    const deactivateIds = [93, 96, 102, 103];
    for (const s of list) {
      if (deactivateIds.includes(s.id) && s.active) {
    ...
    // Also deactivate old Product Manager snippets by name
    for (const s of list) {
      if (s.name && (s.name.includes('Product Manager') || s.name === 'SEO + Recent Products') && s.active) {
        await fetch(WP_URL + '/wp-json/code-snippets/v1/snippets/' + s.id, {
          method: 'PUT', headers: { 'Authorization': 'Basic ' + auth, 'Content-Type': 'application/json' },
          body: JSON.stringify({ active: false })
        });
        console.log('Deactivated by name:', s.id, s.name);
      }
    }
    ...
    await fetch(WP_URL + '/wp-json/code-snippets/v1/snippets', {
      method: 'POST', headers: { 'Authorization': 'Basic ' + auth, 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: 'Purge FinalImg', code: purge, active: true, scope: 'global' })
    });
