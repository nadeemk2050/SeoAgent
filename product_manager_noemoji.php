...
add_filter('the_content', function($content) {
    ...
    if ($p_type) {
        $badge = $p_type === 'buy' ? '[BUY] Buy' : '[SELL] Sell';
...
add_shortcode('product_admin', function() {
    ...
    if ($action === 'push_fb' && isset($_GET['post_id'])) {
        ...
        if (!$auth_ok) {
            ...
            if ($result) {
                $output .= '<div style="background:#d4edda;color:#155724;padding:12px;border-radius:6px;margin-bottom:15px;">[OK] Posted to Facebook! <a href="https://www.facebook.com/profile.php?id=' . FB_PAGE_ID . '" target="_blank" style="color:#155724;font-weight:600;">View Post</a></div>';
            } else {
    ...
    if ($show_form && $mode !== 'pw') {
        ...
        $output .= '<h3 style="margin-top:0;">' . ($mode === 'edit' ? '[EDIT] Edit Product' : '[ADD] Add New Product') . '</h3>';
        ...
        $output .= '<input type="submit" value="' . ($mode === 'edit' ? '[SAVE] Save' : '[QTY] Publish') . '" style="background:#0d954d;color:#fff;padding:12px 25px;border:none;border-radius:6px;font-size:1rem;font-weight:600;cursor:pointer;">';
...
function updImgIds() {
    ...
    $output .= '<p style="margin-bottom:15px;"><a href="/manage-products/?act=add" style="display:inline-block;background:#0d954d;color:#fff;padding:10px 25px;border-radius:6px;text-decoration:none;font-weight:600;">[ADD] Add New Product</a></p>';
    ...
    if (empty($posts)) {
        ...
        foreach ($posts as $p) {
            ...
            $output .= '<a href="/manage-products/?act=delete&del_id=' . $pid . '" style="background:#fee;color:#c00;padding:4px 8px;border-radius:4px;text-decoration:none;font-size:0.8rem;">[DEL] Delete</a>';
