...
async function run() {
  const phpCode = `add_action('wp_footer', function() {
    ...
    #g-icon-email svg { fill: #b8cddb; } /* Light grey-blue */
    #g-icon-phone svg { fill: #364147; } /* Dark charcoal */
    #g-icon-chat svg { fill: #bce0fd; } /* Light sky blue */
    </style>

    <div id="global-sticky-sidebar">
        <!-- Email SVG -->
        ...
        <!-- Phone SVG -->
        <a id="g-icon-phone" href="tel:00971554779240" title="Call Us">
        ...
        <!-- Chat/WhatsApp SVG -->
        <a id="g-icon-chat" href="https://wa.me/971529244592" title="WhatsApp Chat" target="_blank" rel="noopener">
    ...
    <script>
    // Remove any duplicate old sidebars that were injected inside page contents
    document.addEventListener("DOMContentLoaded", function() {
        var oldSidebars = document.querySelectorAll(".custom-sticky-sidebar");
        oldSidebars.forEach(function(sb) {
            sb.remove();
        });
    });
    </script>
  ...
  try {
    const rSnippet = await fetch(site.url + 'wp-json/code-snippets/v1/snippets', {
      ...
      body: JSON.stringify({
        name: 'Global Sticky SVG Sidebar',
        code: phpCode,
        active: true,
        scope: 'global'
      })
    ...
    await fetch(site.url + 'wp-json/code-snippets/v1/snippets', {
      ...
      body: JSON.stringify({ name: 'Purge Cache 12', code: purgeCode, active: true, scope: 'global' })
    });
