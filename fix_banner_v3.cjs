/**
 * BANNER V3 - Inject via WordPress content filter with Astra theme hooks
 */
...
const bannerCode = `
// Inject SEO banner at top of Products archive page
add_action('astra_primary_content_top', function() {
    if (!is_home()) return;
    ?>
    <div style="max-width:1100px; margin:0 auto 30px; padding:20px; background:#f9fffb; border-radius:12px; border:1px solid #d4edda;">
        ...
        <p style="color:#444; font-size:1.05rem; line-height:1.8; margin-bottom:15px;">
            ...
            Browse our latest product listings below.
        </p>
        <figure style="margin:15px 0; text-align:center;">
            <img src="https://alshaabalwaseem.com/wp-content/uploads/2026/05/logo.png" 
                 alt="scrap products for sale UAE - premium scrap materials" 
                 style="max-width:100%; height:auto; border-radius:8px;" />
            <figcaption style="color:#888; font-size:0.85rem; margin-top:8px;">Premium scrap products for sale UAE.</figcaption>
        </figure>
...
async function run() {
  try {
    console.log('📋 Updating banner snippet...');
    ...
    for (const s of list) {
      if (s.name && s.name.includes('Banner') && s.active) {
        ...
        console.log('   🔄 Deactivated old:', s.name);
      }
    }
    ...
    await fetch(WP_URL + '/wp-json/code-snippets/v1/snippets', {
      method: 'POST',
      headers: { 'Authorization': 'Basic ' + auth, 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: 'Purge BannerV3', code: purgeCode, active: true, scope: 'global' })
    });
