...
function post_to_facebook($pid) {
    ...
    if ($p_type || $p_qty || $p_country) {
        ...
        if ($p_country) $msg .= ' | [Dest] ' . $p_country;
    }
    $msg .= "\n\n" . $desc . "\n\n " . $permalink;
...
add_filter('the_content', function($content) {
    ...
    if ($p_country) $html .= '<div>[Dest] <strong>Destination:</strong> ' . esc_html($p_country) . '</div>';
    if ($p_port) $html .= '<div>[Port] <strong>Port:</strong> ' . esc_html($p_port) . '</div>';
    if ($p_terms) $html .= '<div style="width:100%;padding-top:6px;border-top:1px dashed #d4edda;color:#555;">[Terms] <strong>Terms:</strong> ' . esc_html($p_terms) . '</div>';
    $html .= '</div>';
...
add_shortcode('product_admin', function() {
    ...
    if ($show_form && $mode !== 'pw') {
        ...
        // Country
        $output .= '<input type="text" name="product_country" placeholder="[Dest] Destination Country / CNF" value="' . esc_attr($edit_country) . '" style="padding:12px;border:2px solid #e0e0e0;border-radius:6px;font-size:0.9rem;">';
        // Port
        $output .= '<input type="text" name="product_port" placeholder="[Port] Port of Loading" value="' . esc_attr($edit_port) . '" style="padding:12px;border:2px solid #e0e0e0;border-radius:6px;font-size:0.9rem;">';
        $output .= '</div>';
        // Terms
        $output .= '<textarea name="product_terms" rows="2" placeholder="[Terms] Other Terms (payment, inspection, packaging...)" style="padding:12px;border:2px solid #e0e0e0;border-radius:6px;font-size:0.9rem;">' . esc_textarea($edit_terms) . '</textarea>';
...
function updImgIds() {
    ...
    if (empty($posts)) {
        ...
        foreach ($posts as $p) {
            ...
            if ($p_country) $meta_parts[] = '[Dest] ' . $p_country;
            if ($p_port) $meta_parts[] = '[Port] ' . $p_port;
            if (!empty($meta_parts)) {
