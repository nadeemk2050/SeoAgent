...
// Build a clean, complete product_admin shortcode with create, edit, delete, password gate, listing
// No emojis, no Facebook, no the_content filter
const code = `add_action('wp_ajax_upload_product_img','handle_img_upload');
...
async function main() {
  const r = await fetch(WP_URL + '/wp-json/code-snippets/v1/snippets', {
    ...
    body: JSON.stringify({ name: 'PM_Full_Restored', code, active: true, scope: 'global' })
  });
  const d = await r.json();
  console.log('ID:', d.id, 'Active:', d.active, 'Error:', d.code_error ? JSON.stringify(d.code_error).substring(0, 200) : 'none');
  
  if (d.active) {
    // Deactivate old competing snippets
    for (const id of [168, 144, 146, 147, 157, 163, 164]) {
    ...
    console.log('Page working:', !text.includes('[product_admin]') && text.includes('Add New Product'));
  }
}
